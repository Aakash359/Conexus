...
import { ConversationStore, conversationStoreInstance } from '../stores/message-center/conversation-store'
...
const handleIncomingCall = (data, caller, autoAnswer) => {
  ...
  videoStore.callRecieved({ sessionId: data.sessionId, token: data.token }, caller, autoAnswer)
}
  ...
  switch (action) {
    ...
    case 'new-message':
      if (data.conversationId) {
        handleNewMessage(data.conversationId, data.title, data.description, isAppInFocus)
      }
      break
    ...
    case 'call-complete':
    case 'call-connected':
      (Actions.currentScene === ScreenType.HCP_PHONE_CALL_LIGHTBOX) && Actions.pop()
      break
    default:
  }
}
const onOpened = (openResult) => {
  ...
  const { action, caller, ...data } = openResult.notification.payload.additionalData
  log.info(`MSG: ${action} (OnOpened: ${isAppInFocus})`, data, caller, openResult)
...
const respondPushReceived = (pushId: string) => {
  return conversationStoreInstance.pushReceived(pushId)
    .catch(err => log.info('Push received error', err))
}

const respondPingRequest = (pollingUserId: string) => {
  return userService.updateVideoSession(pollingUserId)
    .catch(err => log.info('Respond ping request error', err))
}


const handleNewMessage = (conversationId: string, alertTitle, alertDescription, isAppInFocus: boolean) => {
  try {

    if (isAppInFocus) {
      if (conversationStoreInstance.activeConversation && conversationStoreInstance.activeConversation.conversationId === conversationId) {
        conversationStoreInstance.loadActiveConversation(conversationId).then(
          (unreadCount) => {
            log.info('Notications', 'loadActiveConversation', 'complete')
          })
          .catch((error) => log.info('UserStore', 'loadActiveConversation', 'error', error))
      }

      if (Actions.currentScene === ScreenType.MESSAGE_CENTER.CONVERSATION_LIST || Actions.currentScene === ScreenType.MESSAGE_CENTER.NURSE_CONVERSATION_LIST) {

        conversationStoreInstance.load().then(
          (unreadCount) => {
            log.info('Notications', 'Conversations', 'load')
          }
        )
          .catch((error) => log.info('Notications', 'Conversations', 'load', 'error', error))

      } else {

        conversationStoreInstance.loadUnreadCount()
          .then(
            (unreadCount) => {
              log.info('Notications', 'Conversations', 'UnreadCount', unreadCount)
            }
          )
          .catch((error) => log.info('Notications', 'Conversations', 'LoadUnreadCount', 'error', error))
      }
      ...
      const i = setInterval(() => {
        ...
        if (Actions.currentScene === ScreenType.NURSES.HOME) { // || Actions.currentScene === ScreenType.FACILITIES.REVIEW_HOME) {
          clearInterval(i);
          Actions[ScreenType.MESSAGE_CENTER.CONVERSATION]({ conversationId })
        }
      }, 100)
...
const onRegistered = (notifData) => { console.log('Device Registered: ', notifData) }
const onIds = (device) => {

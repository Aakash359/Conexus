...
import { ViewProperties, StyleSheet, View, FlatList, Alert, Platform, RefreshControl } from 'react-native'
import { FacilityNeedsStore, FacilityNeedsModel, NeedSummaryModel, NeedDetailModel, UserStore } from '../../../stores'
...
@inject('userStore', 'facilityNeedsStore')
@observer
export class NeedsContainer extends React.Component<NeedsContainerProps, NeedsContainerState>  {
    ...
    get selectedFacility(): typeof FacilityNeedsModel.Type {
        ...
        if (userStore.selectedFacilityId) {
            return facilityNeedsStore.facilities.find(i => i.facilityId === userStore.selectedFacilityId);
        }
        ...
        needsStorePromise
            .then(() => {
                if (this.mounted) {
                    ...
                    log.info('NeedContainer', 'Not mounted so doing nothing after data loaded.')
                }
            })
            .catch((error) => {
                if (this.mounted) {
                ...
                Alert.alert('Error', "We are having trouble loading needs. Please try again.")
            })
    }

    renderNeedBodyMetric(title: string, description: string) {
        return (
            <View key={`need-metric-${title}-${description}`} style={{ paddingHorizontal: 6, alignItems: 'center', justifyContent: 'center' }}>
                <Text style={{ ...AppFonts.bodyTextXtraSmall, paddingBottom: 4 }}>{title}</Text>
    ...
    renderNeedBody(need: typeof NeedDetailModel.Type) {
        return need.stats.map(stat => this.renderNeedBodyMetric(stat.title, stat.description));
    }

    renderCardButton(options: { id: string, title: string, onPress: () => any, isLastCard: boolean, firstButton: boolean, lastButton: boolean }) {
        const paddingBottom = options.isLastCard ? 18 : 0
        const buttonStyle = styles[`cardButton${options.firstButton ? 'First' : ''}${options.lastButton ? 'Last' : ''}`]
        return <View key={`need-${options.id}-details`} style={[{ flex: 1, marginBottom: 6, paddingBottom }]}>
            <Button style={buttonStyle} onPress={options.onPress}><Text style={styles.cardButtonText}>{options.title}</Text></Button>
        </View>
    }
    ...
    renderNeed(need: typeof NeedDetailModel.Type, index: number, needs: typeof NeedDetailModel.Type[]) {
        ...
        result.push(
            <View key={`need-${need.needId}`} style={{ flex: 1, margin: 8, paddingBottom }}>
                <ListItem onPress={this.toggleExpandedNeedId.bind(this, need.needId)} style={cardStyle}>
    ...
    renderNeedSection({ item, index }) {
        ...
        if (result.length) {
            result.unshift(<ViewHeader key={`section-item-${index}`} title={needSummary.Status} style={{ paddingTop: 12, paddingBottom: 6, backgroundColor: AppColors.baseGray, borderBottomWidth: 0 }} />)
        }
    ...
    render() {
        ...
        const { facilityNeedsStore, userStore } = this.props
        const facility = facilityNeedsStore.getSnapshot().facilities.find(i => i.facilityId === userStore.selectedFacilityId);

...
import { Icon, Text, ListItem,Right, Body} from 'native-base'
import { ViewProperties, StyleSheet, View, FlatList, RefreshControl } from 'react-native'
import { Actions } from 'react-native-router-flux'
import { inject, observer } from 'mobx-react'
import { AppFonts, AppColors, AppSizes } from '../../../theme'
import { ScreenType, showYesNoAlert } from '../../../common'
import { QuestionSectionModel, FacilityQuestionsStore, FacilityModel } from '../../../stores/facility/'
import { UserStore } from '../../../stores/'
import { FacilitySelectionContainer, FacilitySelectionItem } from '../../../components'
...
@inject('userStore', 'facilityQuestionsStore')
@observer
export class CatalogContainer extends React.Component<CatalogContainerProps, CatalogContainerState>  {
    ...
    get selectedFacility() {
        return this.props.facilityQuestionsStore.facilities.find(i => i.facilityId === this.selectedFacilityId);
    }
    ...
    get facilities(): FacilitySelectionItem[] {
        ...
        return facilityQuestionsStore.facilities.map((f: typeof FacilityModel.Type) => {return {facilityId: f.facilityId, facilityName: f.facilityName, photoUrl: f.facPhotoUrl || ''}})
    }
    ...
    private load(refreshing: boolean = false) {
        ...
        facilityQuestionsStore.load(needId || '')
            .then(() => {
                log.info('FacilityQuestionStore Loaded', facilityQuestionsStore.facilities)
                this.setState({ refreshing: false, loading: false })
            })
            .catch(error => {
                log.info(error)
                showYesNoAlert({ title: `We're Sorry`, message: 'An error occurred while loading available questions.', onYes: this.load.bind(this, refreshing), yesTitle: 'Retry', noTitle: 'Canel' })
                this.setState({ refreshing: false, loading: false })
            })
    }
        ...
        return (
            <ListItem
                ...
                <Body style={StyleSheet.flatten([styles.itemSection, styles.body])}>
                    <Text style={AppFonts.listItemTitleTouchable}>{item.sectionTitle}</Text>
                    <Text style={AppFonts.listItemDescription}>{`${questionCount} question${questionCount > 1 ? 's' : ''}`}</Text>
                </Body>
                <Right>
        ...
        return (
            <FacilitySelectionContainer
                ...
                <FlatList
                    ...
                    ListFooterComponent={() => { return <View key="spacer" style={{ height: 120 }} /> }}
                    renderItem={this.renderSection.bind(this)}
                    data={this.sections}
                />
                }
                <ScreenFooterButton title="Add Question" onPress={this.showNewQuestion.bind(this)} />
            </FacilitySelectionContainer>
        )
    }
}

...
import { ViewProperties, StyleSheet, View, Platform, Animated, Easing, ViewStyle, TouchableOpacity } from 'react-native'
...
@observer
export class SortableQuestionRow extends React.Component<SortableQuestionProps, any> {
    _style: ViewStyle;
    ...
    constructor(props, state) {
        ...
        this._style = Platform.OS === 'android' ? this._getAndroidStyle() : this._getIosStyle();
    }
    _getIosStyle() {
        return {
            transform: [{
        ...
        if (!videoUrl) {
            return (
                <Circle size={46} color={AppColors.white} style={styles.circle}>
                    <ConexusIcon style={styles.playIcon} name={"cn-play"} color={AppColors.lightBlue} size={24} />
                </Circle>
            )
        }
    ...
    _renderIcon() {
        ...
        return (
            <View style={[styles.iconContainer, {marginRight: 12}]}>
                ...
                {
                    editing && <ConexusIcon name={"cn-hamburger"} color={AppColors.blue} size={16} style={{}} />
                }
            </View>
        )
    }
    ...
    _renderDeleteIcon() {
        return (
            <TouchableOpacity style={[styles.iconContainer, {marginLeft: 12}]} onPress={this._deleteQuestion.bind(this)}>
            ...
            <Animated.View style={[
                ...
                {editing &&
                    <View style={styles.questionDetailsButton}>
                        <Text style={styles.questionTextEditing}>{text}{this._style.marginBottom}</Text>
                    </View>
                }
                {!editing &&
                    <TouchableOpacity style={styles.questionDetailsButton} onPress={editing ? null : this._openQuestion.bind(this)}>
                        <Text style={styles.questionText}>{text}{this._style.marginBottom}</Text>
                    </TouchableOpacity>
                }
                {editing && allowDeleteQuestion &&

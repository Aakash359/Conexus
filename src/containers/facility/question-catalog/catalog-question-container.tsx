...
export class CatalogQuestionContainer extends React.Component<
  CatalogQuestionContainerProps,
  CatalogQuestionContainerState
> {
      ...
      <TouchableOpacity onPress={this.openUnitSelector.bind(this)}>
        <View style={styles.chooserField}>
          <View style={{flex: 1}}>
            ...
            {!!unitType && (
              <Text style={styles.chooserFieldText}>{unitType.unitName}</Text>
            )}
          </View>
          <ConexusIcon
            style={styles.chooserFieldIcon}
            size={16}
            name="cn-dropdown"
      ...
      <KeyboardAvoidingView style={{flex: 1}}>
        ...
        {this._question.canPlayQuestion && (
          <ScreenFooterButton
            title="Play Question"
            onPress={() =>
              // Actions[ScreenType.FACILITIES.QUESTION_PLAYBACK_LIGHTBOX]
              ({videoUrl: this._question.tokBoxArchiveUrl})
            }
          />
        )}

...
export class CatalogSectionContainer extends React.Component<
  CatalogSectionContainerProps,
  CatalogSectionContainerState
> {
  get onSave() {
  ...
  get section(): typeof QuestionSectionModel.Type {
    const {userStore, questionSectionId, facilityQuestionsStore, needId} =
      this.props;
    ...
    return facilityQuestionsStore.findQuestionSection(
      userStore.selectedFacilityId,
      questionSectionId,
    );
  }
  ...
  renderUnitHeader() {
    const {editing} = this.state;
    const questionCount =
      this.section.questions.length + this.section.defaultQuestions.length;
    ...
    const tabs = [
      {
        ...
        badge: this.showDefault
          ? ''
          : this.getQuestionsBadge(this.section.defaultQuestions, true),
      },
      {
        id: 'other',
        title: 'Other Questions',
        badge: this.showDefault
          ? this.getQuestionsBadge(this.section.questions, false)
          : '',
      },
    ];
  ...
  getQuestionsBadge(
    questions: typeof QuestionModel.Type[],
    ...
    return (
      <Container style={{flex: 1}}>
        ...
        {!editing && (
          <ScreenFooterButton
            title="Add Question"
            onPress={this.newQuestion.bind(this)}
          />
        )}
      </Container>
    );

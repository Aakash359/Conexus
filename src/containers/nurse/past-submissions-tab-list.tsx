...
export class PastSubmissionsTabList extends React.Component<
  PastSubmissionsTabListProps,
  PastSubmissionsTabListState
> {
  ...
  _renderSubmissionItem(
    submission: typeof NurseSubmissionModel.Type,
    index: number,
  ) {
    return (
      <ListItem
        key={submission.submissionId.toString() + index}
        avatar
        ...
        <Left>
          <Avatar
            ...
            size={48}
            borderWidth={1}
            source={submission.facilityImage}
          />
        </Left>
        <Body style={styles.body}>
          ...
          <Text style={[{...AppFonts.listItemDescription}]}>
            {submission.display.description}
          </Text>
        </Body>
      </ListItem>
    );
  ...
  _renderSubmissions(
    completed: typeof NurseSubmissionModel.Type[],
    closed: typeof NurseSubmissionModel.Type[],
  ) {
    ...
    return (
      <ScrollView
        ...
        {!!submissions.length &&
          submissions.map(this._renderSubmissionItem.bind(this))}
      </ScrollView>
    );
  }

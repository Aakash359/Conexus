...
import { StyleSheet, View, ViewProperties, ActivityIndicator, Platform } from 'react-native'
...
@inject('videoStore')
@observer
export class QuestionAnswerRecorder extends React.Component<QuestionAnswerRecorderProps, QuestionAnswerRecorderState> {
    ...
    private readonly answerStep: Step = {
        ...
        initStep: () => {
            ...
            InCallManager.setKeepScreenOn(true)
            this.beginCountdown(this.answerStep.secondsLeft, this.answerStep.onComplete)
            return Promise.resolve()
        ...
        onComplete: () => {
            ...
            if (this.answerStep.stepState.mode === 'thinking') {
                ...
                this.forceUpdate(() => {
                    this.props.videoStore
                        ...
                        .then(() => {
                            ...
                            this.beginCountdown(this.props.answerLengthSeconds || 120, this.answerStep.onComplete)
                        })
                        .catch(error => {
                            log.info('Question Answer Recorder', 'Record session failure', error)
                ...
                this.props.videoStore.stopRecordingSession()
                    .then((archive: any) => {
                        log.info('Question Answer Recorder', 'Recording complete', 'Disconnecting videoStore')
                        if (archive.Size == 0) {
                            showApiErrorAlert({
        ...
        render: () => {
            ...
            if (this.answerStep.stepState.mode === 'thinking') {
                showCountdown = true
                footerButton = <ScreenFooterButton hideGradient title="Answer" onPress={() => {
                    this.clearCountdown()
            ...
            return (
                <View style={{ flex: 1, backgroundColor: AppColors.black }}>
                    <View style={{ flex: 1, padding: 20 }}>
                        <Text style={{ marginTop: 200, textAlign: 'center', ...AppFonts.bodyTextLarge, color: 'white' }}>Yay! You answered a question.</Text>
                        <Text style={{ marginTop: 12, textAlign: 'center', ...AppFonts.bodyTextNormal, color: 'white' }}>Take a break and continue when ready</Text>
                        <ScreenFooterButton hideGradient title="Next Question" onPress={this.breakStep.onComplete.bind(this)} />
                    </View>
                </View>
            )
        },
    ...
    private readonly errorStep: Step = {
        ...
        render: () => {
            ...
            return (
                <View style={{ flex: 1, backgroundColor: AppColors.black }}>
                    <View style={{ flex: 1, padding: 20 }}>
                        <Text style={{ marginTop: 200, textAlign: 'center', ...AppFonts.bodyTextLarge, color: 'white' }}>An error occurred while presenting this question.</Text>
                        <Text style={{ marginTop: 12, textAlign: 'center', ...AppFonts.bodyTextNormal, color: 'white' }}>We'll skip this one for now. Click continue when ready for the next question.</Text>
    ...
    constructor(props: QuestionAnswerRecorderProps, state: QuestionAnswerRecorderState) {
        super(props, state)
    ...
    componentDidMount() {
        ...
        this.props.videoStore.initVideoSession().then(() => {
            if (!(this.props.questionUrl).trim()) {
                log.info('Question Answer Recorder', 'Empty question url. Can not play question')
                this.goNextStep(this.errorStep)
            } else {
            ...
            if (nextStep) {
                nextStep.initStep()
                    .then(
                        ...
                        (error) => {
                            log.info('Question Answer Recorder', 'Go Next Step', 'ERROR', error)
    ...
    beginCountdown(secondsLeft: number, onComplete: () => any) {
        ...
        if (secondsLeft > 0) {
            this.countdownTimeoutHandle = setTimeout(() => {
                ...
                this.forceUpdate(() => { this.beginCountdown(secondsLeft, onComplete) })
            }, 1000)
        } else {
            onComplete()
        }
    ...
    renderQuestionHeader(showCountdown) {
        const { createdByName, createdByTitle, createdByUrl, questionText } = this.props
        return (

...
import { Alert, StyleSheet, ScrollView, Platform, TextInput, Keyboard, EmitterSubscription, Animated, RefreshControl, ActivityIndicator, TouchableHighlight, ViewProperties, KeyboardAvoidingView } from 'react-native'
...
import { ConversationStore, ConversationModel, ConversationMessageModel, conversationStoreInstance } from '../../stores/message-center'
...
@inject('userStore', 'conversationStore', 'videoStore') @observer
export class ConversationContainer extends React.Component<ConversationContainerProps, ConversationContainerState> {
    ...
    get hcpUserId(): string {
        return this.activeConversation ? this.activeConversation.hcpUserId : this.props.recipientUserId;
    }

    get recipientPhotoUrl(): string {
        return this.activeConversation ? this.activeConversation.hcpPhotoUrl : this.props.recipientPhotoUrl;
    }

    get recipientName(): string {
        return this.activeConversation ? this.activeConversation.hcpFirstName + ' ' + this.activeConversation.hcpLastName : this.props.recipientName;
    }

    get canMakePhoneCall(): boolean {
        return this.props.userStore.isFacilityUser && !!this.hcpUserId && !!this.recipientName
    }

    get submissionId(): string {
        return this.activeConversation ? this.activeConversation.submissionId : this.props.submissionId
    }
    ...
    get activeConversation(): typeof ConversationModel.Type {
        const conversationStore: typeof ConversationStore.Type = this.props.conversationStore;
    ...
    constructor(props: ConversationContainerProps, state: ConversationContainerState) {
        super(props, state)
        ...
        if (!props.conversationId && !this.props.submissionId) {
            throw new Error('Neither a conversation was found or a submissionId suplied to start a new conversation. One or the other is required.')
        }
    ...
    onMessageSend(conversationId: string) {
        if (this.props.onMessageSendCallback && this.props.onMessageSendCallback.call) {
            try {
    ...
    playVideoMessage(videoUrl: string, message: typeof ConversationMessageModel.Type) {
    ...
    playAudioMessage(message: typeof ConversationMessageModel.Type) {
        ...
        const parms = { audioUrl: message.audioUrl, videoUrl: undefined, avatarUrl: message.senderPhotoUrl, avatarTitle, avatarDescription }
    ...
    toggleFooterActions = () => {
        ...
        this.setState({ showFooterActions: !showFooterActions }, this.applyMeasurements.bind(this))
    }

    sendVideoMessage = () => {
        Keyboard.dismiss();

        const conversationId = this.activeConversation ? this.activeConversation.conversationId : '';
    ...
    sendTextMessage = () => {
        ...
        return conversationStore.sendTextMessage(conversationId, submissionId, messageText.replace(/\s+$/g, '')).then(
            (conversationId) => {
                this.clearMessageText()
                Keyboard.dismiss()

                log.info(`SubmissionId: ${submissionId} generated conversationId: ${conversationId}`)
    ...
    initVideoCall() {
        ...
        if (this.canMakeVideoCall) {
            videoStore.call(this.submissionId, this.hcpUserId, { title: '', name: this.recipientName, subTitle: '', photo: this.recipientPhotoUrl })
        }
    }
    ...
    loadMessages(refreshing = false) {
        ...
        if (conversationId && !conversationStore.loading) {
            ...
            conversationStore.loadActiveConversation(conversationId).then(
                ...
                (error) => {
                    if (loading) {
                        Alert.alert('Message Center Error', 'We are having trouble loading your conversation. Please try again.')
                    }
    ...
    renderAudioMessageView(message: typeof ConversationMessageModel.Type, index: number) {
        const time = moment(message.messageTimestamp).format('M/D/YYYY h:mm a')
        ...
        return (
            <View key={`message-${message.messageId}-${index}`} style={[style.messageView, style.audioMessageView, message.sentByMe ? style.messageFromMeView : {}]}>
                <TouchableHighlight onPress={this.playAudioMessage.bind(this, message)} style={[style.audioMessageWrapper, message.sentByMe ? style.audioMessageWrapperFromMe : {}]}>
                    <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center' }}>
                        <Circle size={28} color={message.sentByMe ? AppColors.white : AppColors.blue} style={{ alignItems: 'center', justifyContent: 'center' }}>
                            <ConexusIcon style={{ paddingLeft: 3 }} name="cn-play" size={18} color={message.sentByMe ? AppColors.blue : AppColors.white} />
                        </Circle>
                        <Text style={[style.audioMessageText, message.sentByMe ? style.audioMessageTextFromMe : {}]}>Audio Message</Text>
                    </View>
                </TouchableHighlight>
                <Text style={[style.caption, message.sentByMe ? style.captionFrom : {}]}>{caption}</Text>
            </View>
        )
    }

    renderVideoMessageView(message: typeof ConversationMessageModel.Type, index: number) {
        const time = moment(message.messageTimestamp).format('M/D/YYYY h:mm a')
        const caption = `${message.senderFirstName} ${message.senderLastName}, ${time}`
        ...
        return (
            <View key={`message-${message.messageId}-${index}`} style={[style.messageView, style.videoMessageView, message.sentByMe ? style.messageFromMeView : {}]}>
                <TouchableHighlight onPress={this.playVideoMessage.bind(this, message.tokBoxArchiveUrl, message)} style={[style.videoMessageWrapper, message.sentByMe ? style.videoMessageWrapperFromMe : {}]}>
                    <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center' }}>
                        <Circle size={28} color={message.sentByMe ? AppColors.white : AppColors.blue} style={{ alignItems: 'center', justifyContent: 'center' }}>
                            <ConexusIcon style={{ paddingLeft: 3 }} name="cn-play" size={18} color={message.sentByMe ? AppColors.blue : AppColors.white} />
                        </Circle>
                        <Text style={[style.videoMessageText, message.sentByMe ? style.videoMessageTextFromMe : {}]}>
                            Video Message {message.read ? "" : " *"}</Text>
                    </View>
                </TouchableHighlight>
                <Text style={[style.caption, message.sentByMe ? style.captionFrom : {}]}>{caption}</Text>
            </View>
    ...
    renderTextMessageView(message: typeof ConversationMessageModel.Type, index: number) {
        const time = moment(message.messageTimestamp).format('M/D/YYYY h:mm a')
        const caption = `${message.senderFirstName} ${message.senderLastName}, ${time}`

        return (
            <View key={`message-${message.messageId}-${index}`} style={[style.messageView, style.textMessageView, message.sentByMe ? style.messageFromMeView : {}]}>
                <View style={[style.textMessageTextWrapper, message.sentByMe ? style.textMessageTextWrapperFromMe : {}]}><Text selectable style={[style.textMessageText, message.sentByMe ? style.textMessageTextFromMe : {}]}>{message.messageText}</Text></View>
                <Text style={[style.caption, message.sentByMe ? style.captionFrom : {}]}>{caption}</Text>
            </View>
        )
    }
    ...
    renderMessageScrollView() {
        return (<ScrollView style={style.messageListView}
    ...
    renderMessageList() {
        ...
        if (this.showableMessages.length) {
            return (
                <Animated.View style={[{ flex: 1, alignItems: 'stretch', justifyContent: 'center', backgroundColor: AppColors.baseGray }]}>
            ...
            return (
                <Animated.View style={[{ flex: 1, alignItems: 'stretch', justifyContent: 'center', backgroundColor: AppColors.baseGray }]}>
                    <ActivityIndicator color={AppColors.blue} style={{ flex: 1 }}></ActivityIndicator>
                </Animated.View>
            )
        }
        ...
        return (
            <Animated.View style={[{ flex: 1, alignItems: 'stretch', justifyContent: 'center', backgroundColor: AppColors.baseGray }]}>
                {
                    <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', paddingBottom: 180 }}>
                        {
                            !!this.recipientName &&
    ...
    renderFooter() {
        ...
        return (
            <Animated.View key="footer" style={[footerStyle.footerView]}>
                <Animated.View style={[footerStyle.inputRow, { height: this.footerInputHeight }]}>
                    <View style={footerStyle.inputWrapper}>
                        <ConexusIconButton iconName="cn-more" iconSize={16} color={AppColors.blue} onPress={this.toggleFooterActions.bind(this)} style={footerStyle.moreButton}></ConexusIconButton>
                        <TextInput underlineColorAndroid={'transparent'} value={messageText} selectionColor={AppColors.blue} onChangeText={this.setMessageText.bind(this)} returnKeyType="default" editable={true} multiline={true} style={footerStyle.input} onContentSizeChange={this.onInputContentSizeChange.bind(this)} />
                    </View>
                    <Button style={StyleSheet.flatten([footerStyle.sendButton, !sendEnabled && footerStyle.sendButtonDisabled])} onPress={this.sendTextMessage.bind(this)}>
                        <Text style={[footerStyle.sendButtonText, !sendEnabled && footerStyle.sendButtonTextDisabled]}>Send</Text>
                    </Button>
                </Animated.View>
                {!!showFooterActions &&
                    <View style={[footerStyle.actionsRow]}>
                        <ConexusIconButton disabled={!showFooterActions} title="Video Message" iconName="cn-video-message" iconSize={24} color={AppColors.blue} onPress={this.sendVideoMessage.bind(this)} style={footerStyle.actionRowButton} textStyle={footerStyle.actionButtonText}></ConexusIconButton>
                        {this.canMakeVideoCall && <ConexusIconButton disabled={!showFooterActions} title="Video Call" iconName="cn-video" iconSize={24} color={AppColors.blue} onPress={this.initVideoCall.bind(this)} style={footerStyle.actionRowButton} textStyle={footerStyle.actionButtonText}></ConexusIconButton>}
                        {this.canMakePhoneCall && <ConexusIconButton disabled={!showFooterActions} title="Phone Call" iconName="cn-phone" iconSize={24} color={AppColors.blue} onPress={this.initPhoneCall.bind(this)} style={footerStyle.actionRowButton} textStyle={footerStyle.actionButtonText}></ConexusIconButton>}
                    </View>
                }
            </Animated.View>
        )
    ...
    render() {
        ...
        return (
            <SafeAreaView style={[{ flex: 1, backgroundColor: 'white' }]}  >
                <Animated.View style={{ flex: 1, paddingBottom: this.paddingBottom }} onLayout={this.onLayout} ref={c => { this.view = c; }}>
...
const style = StyleSheet.create({
    ...
    textMessageTextWrapper: {
        flex: 1,
        flexDirection: 'row',
        maxWidth: AppSizes.screen.width * .65,
        minWidth: AppSizes.screen.width * .65,
        padding: 8,
    ...
    videoMessageWrapper: {
        ...
        flexDirection: 'row',
        alignItems: 'center',
        maxWidth: AppSizes.screen.width * .5,
        minWidth: AppSizes.screen.width * .5,
        padding: 8,
        paddingHorizontal: 10,

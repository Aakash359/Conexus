...
import { Avatar, ViewHeader, Circle, FacilitySelectionContainer } from '../../components'
import { observer, inject } from 'mobx-react'
import { ScreenType } from '../../common'
import { Actions } from 'react-native-router-flux'
import { UserStore } from '../../stores'
import { AppFonts, AppColors } from '../../theme'
import { ConversationStore, ConversationPositionModel } from '../../stores/message-center'
...
@observer
export class FacilityMessageCenterContainer extends React.Component<MessageCenterProps, MessageCenterState> {
    ...
    get selectedFacility() {
        return this.props.conversationStore.facilities.find(i => i.facilityId === this.selectedFacilityId)
    }
    ...
    renderConversationItem({ item, index }) {
        ...
        result.push(<ViewHeader first={i === 0} key={needKey} title={position.display.title} description={position.display.description} />)
            ...
            result.push(
                <ListItem key={listItemKey}
                    ...
                    <Left style={itemStyles.itemSection}>
                        <Circle size={10} color={conversation.unreadCount ? AppColors.red : 'transparent'} style={{ marginTop: 19, marginRight: 4 }} />
                        <Avatar size={48} source={conversation.hcpPhotoUrl} />
                    </Left>
                    <Body style={StyleSheet.flatten([itemStyles.itemSection, itemStyles.body])}>
                        <Text style={AppFonts.listItemTitleTouchable}>{conversation.display.title}</Text>
                        <Text style={AppFonts.listItemDescription}>{conversation.display.description}</Text>
                    </Body>
                    <Right style={itemStyles.itemSection}>
                        <Text style={[{ ...AppFonts.bodyTextXtraSmall }, { textAlign: 'right' }]}>{lastMessageDate}</Text>
                    </Right>
                </ListItem>
            )
        })
    ...
    load(refreshing: boolean = false) {
        ...
        conversationStore.load().then(
            ...
            .catch(error => {
                this.setState({ refreshing: false, loading: false })
                log.info(error)
                Alert.alert('Message Center Error', 'We are having trouble loading your conversations. Please try again.')
            })
    }

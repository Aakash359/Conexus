...
import { View, ViewProperties, StyleSheet, ActivityIndicator, TouchableOpacity, Platform, ViewStyle, Image } from 'react-native'
...
@observer
export class ConexusVideoPlayer extends React.Component<ConexusVideoPlayerProps, ConexusVideoPlayerState>  {
    ...
    get errorDisplayText(): string {
        return this.props.errorDisplayText || `This video is currently unavailable.`
    }
    ...
    volumeListener = SystemSetting.addVolumeListener(_.debounce((data) => {
        const volume = isAndroid ? data['music'] : data.value
        this.setState({ volume: data.value })
    }, 200))

    constructor(props: ConexusVideoPlayerProps, context: ConexusVideoPlayerState) {
        super(props, context)
    ...
    getVolumeStyle() : ViewStyle {
        ...
        if (location === 'top-right') {
            return { position: 'absolute', zIndex: 101, right: 0, width: 150, top: 0, padding: 10, flexDirection: 'row', backgroundColor: 'transparent' }
        }
            
        return { position: 'absolute', zIndex: 101, left: 0, width: 150, top: 0, padding: 10, flexDirection: 'row', backgroundColor:  'transparent' }
    }
    ...
    renderLoading() {
        ...
        let indicator = <ActivityIndicator color={this.props.loadingColor || AppColors.blue} style={{ flex: 1 }} />
        ...
        return (
            <View style={[styles.container]}>{indicator}</View>
        )
    }
    ...
    renderActions() {
        ...
        const replayButton = { title: this.replayTitle, onPress: () => this.replay() }
        ...
        if (this.hasError) {
            ...
            buttons = [
                replayButton,
                ...menuButtons.filter(i => i.showOnError)
            ]

        } else if (this.isPaused) {
            // Add Play to the top of the list
            buttons = [
                { title: 'Play', onPress: () => this.togglePause() }
            ]
            ...
            if (this.isPlaying && this.props.showActionButtonWhilePlaying) {
                ...
                if (btnDetails) {
                    return (
                        ...
                        <ScreenFooterButton hideGradient title={btnDetails.title} onPress={btnDetails.onPress}></ScreenFooterButton>
        ...
        return (
            <View style={[styles.overlay]}>
                <View style={[styles.overlayContent, this.hasError ? this.props.overlayContentWithErrorStyle : this.props.overlayContentStyle]} >
                    {this.hasError &&
                        <IconTitleBlock textColor={AppColors.white} iconColor={AppColors.white} style={styles.errorHeader} iconName={this.hideErrorIcon ? undefined : this.errorIconName} text={this.errorDisplayText} />
                    }
                    <View style={[styles.overlayButtons, this.hasError ? styles.overlayButtonsWithError : null]}>
                        {
                            buttons.map((b, i) => <Button key={'mb' + i} rounded transparent style={styles.overlayButton} onPress={b.onPress.bind(this)}>
                ...
                {!!actionButton &&
                    ...
                    <ScreenFooterButton hideGradient title={actionButton.title} onPress={actionButton.onPress}></ScreenFooterButton>
    ...
    renderPausedOverlay(): JSX.Element | undefined {
        ...
        if (this.props.renderStoppedOverlay && this.props.renderStoppedOverlay.call) {
    ...
    renderPlayOverlay(): JSX.Element | undefined {
        ...
        if (this.isPlaying && this.props.renderPlayOverlay && this.props.renderPlayOverlay.call) {
    ...
    renderPlayer() {
        return (
            <TouchableOpacity disabled={!this.pausable || this.isPaused} style={[styles.container, this.props.style]} onPress={this.togglePause.bind(this)}>
                {/* {
                !this.hidePlayer &&
                ...
                {!this.hidePlayer && !!this.props.volumeLocation &&
                    <View style={this.getVolumeStyle()}>
                        <Slider
                            ...
                            trackStyle={{ height: 2}}
                            thumbStyle={{ alignItems: 'center', justifyContent: 'center', width: 24, height: 24, borderRadius: 12}}

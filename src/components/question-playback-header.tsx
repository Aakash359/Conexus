...
@observer
export class QuestionPlaybackHeader extends React.Component<QuestionPlaybackHeaderProps, QuestionPlaybackState> {

    lightbox: ConexusLightbox;

    constructor(props: QuestionPlaybackHeaderProps, state: QuestionPlaybackState) {
        super(props, state)
    ...
    render() {
        const { style, showAvatar, avatarTitle, avatarDescription, avatarUrl, showCountdown, countdownTitle, countdownDescription, questionText } = this.props;
        
        if (showAvatar || showCountdown) {
            return (
                <View style={style}>
                    <LinearGradient style={{ height: 240, position: 'absolute', top: 0, left: 0, right: 0 }} colors={['rgba(0, 0, 0, .6)', 'rgba(0, 0, 0, 0)']}></LinearGradient>
                    <View style={[styles.questionHeader]}>
                            ...
                            <View style={styles.avatarContainer}>
                                ...
                                <View style={{ position: 'relative' }}>
                                    <View style={[{ position: 'absolute', top: 1, left: 1 }]}>
                                        ...
                                        <Text style={styles.avatarDescriptionBg}>{avatarDescription}</Text>
                                    </View>
                                    <View style={[]}>
                        ...
                        {showCountdown &&
                            <View style={[countdownStyle.view, {position: 'absolute', right: 19, top: 21 }]}>
                                <Text style={countdownStyle.titleBg}>{countdownTitle}</Text>
                                <Text style={countdownStyle.secondsLeftBg}>{countdownDescription}</Text>
                            </View>
                        }
                        {showCountdown &&
                            <View style={[countdownStyle.view, { right: 20, top: 20 }]}>
                                <Text style={countdownStyle.title}>{countdownTitle}</Text>
                                <Text style={countdownStyle.secondsLeft}>{countdownDescription}</Text>
                            </View>
                        }
                        
                    </View>
                    <View style={{ position: 'relative', ...{marginTop: (showAvatar) ? 12 : 175} }}>

...
import { StyleSheet, View, ViewProperties, StyleProp, ViewStyle } from 'react-native'
import { ConexusIcon } from '../components/conexus-icon';
import variables from '../theme'
import FitImage from 'react-native-fit-image';
import { AppFonts } from '../theme';
import { logger } from 'react-native-logs';
import { ContentListModel, ContentListItemModel } from '../stores/content-list-model';
...
export class ConexusContentList extends Component<ConexusIconListProps> {
    ...
    render() {
        return (
            <View style={this.props.style}>
                {this.props.data.map((item, index) => this.renderContentBlock(item, index))}
            </View>
        )
    }
    ...
    renderContentBlock(item: typeof ContentListModel.Type, index: number) {
        return (
            <View key={`content-block-${index}`} style={StyleSheet.flatten([style.contentBlock, (index + 1) % 2 === 0 ? style.contentBlockEven : {}])}>
    ...
    renderContentBlockList(list: typeof ContentListItemModel.Type[]) {
        ...
        return list.map((item, index) =>
            <View key={`content-block-list-item-${index}`}  style={style.contentBlockListItem} >
    ...
    renderIcon(name: string, iColor: string) {
        return (<ConexusIcon name={name} color={iColor} size={24} style={style.contentBlockListItemIcon} />)
    }

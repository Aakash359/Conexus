...
class CallService {
  ...
  constructor() {
    this._outgoingCallSound = new Sound(
      require('../../components/Images/player/volume.png'),
    );
    this._incomingCallSound = new Sound(
      require('../../components/Images/player/volume.png'),
    );
    this._endCallSound = new Sound(
      require('../../components/Images/player/volume.png'),
    );
  }
  ...
  init() {
    ...
    ConnectyCube.videochat.onAcceptCallListener = this._onAcceptCallListener.bind(
      this,
    );
    ConnectyCube.videochat.onRejectCallListener = this._onRejectCallListener.bind(
      this,
    );
    ConnectyCube.videochat.onStopCallListener = this._onStopCallListener.bind(
      this,
    );
    ConnectyCube.videochat.onUserNotAnswerListener = this._onUserNotAnswerListener.bind(
      this,
    );
    ConnectyCube.videochat.onRemoteStreamListener = this._onRemoteStreamListener.bind(
      this,
    );
  ...
  reportEndCallWithoutUserInitiating(callUUID, reason) {
    ...
    console.log(
      '[CallKitService][reportEndCallWithoutUserInitiating]',
      callUUID,
      reason,
    );
  ...
  onEndCallAction = async data => {
    ...
    if (this.callSession) {
      ...
      const voipIncomingCallSessions = await userDefaults.get(
  ...
  onLoadWithEvents = events => {
    ...
    console.log('[CallKitService][onLoadWithEvents]', {
      callDataToAdd,
      callDataToAnswer,
    });

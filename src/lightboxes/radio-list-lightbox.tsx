...
import { View, Text, StyleSheet, ViewProperties, ActivityIndicator, FlatList, ScrollView } from 'react-native'
...
export class RadioListLightbox extends React.Component<RadioListLightboxProps, RadioListLightboxState> {
    constructor(props) {
        super(props);
    ...
    renderItem({ item, index }) {
        ...
        return (
            <ListItem key={`${i.value}${index}`} style={styles.listItem} onPress={this._selectValue.bind(this, i.value)}>
                {showImages && <Avatar source={i.imageUrl} facility={facilityImages} size={45} style={styles.listItemAvatar} />}
                <View style={styles.listItemTitleWrapper}>
                    <Text style={AppFonts.listItemTitle}>{i.title}</Text>
                </View>
                {!hideSelectedIcon && this.state.value === i.value && <ConexusIcon style={styles.listItemIcon} size={21} color={AppColors.blue} name="cn-check" />}
            </ListItem>
        )
    }
    render() {
        ...
        return (
            <ConexusLightbox closeable style={{ padding: 0 }} title={title} height={380} horizontalPercent={0.9}>
                ...
                {
                    !!actionText && <View style={styles.lightboxFooter}>
                        <ActionButton primary disabled={value == null || value === ''} title={actionText} onPress={this._close.bind(this)}></ActionButton>
                    </View>
                }
            </ConexusLightbox>
        )

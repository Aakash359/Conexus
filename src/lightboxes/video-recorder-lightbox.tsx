...
import { StyleSheet, ViewProperties, ActivityIndicator, StatusBar } from 'react-native'
...
import { ConexusVideoRecorder, ConexusVideoPlayer, ConexusVideoActionButton } from '../components'
...
@inject('videoStore', 'conversationStore') @observer
export class VideoRecorderLightbox extends React.Component<VideoRecorderLightboxProps, VideoRecorderLightboxState> {
  ...
  onPlayError() {
    setTimeout(() => {
      showYesNoAlert({
        ...
        yesTitle: 'Try Again',
        noTitle: 'Cancel',
        onYes: () => { this.setState({ showPlayer: true }) },
        onNo: this.onCanceled.bind(this)
      })
    }, 0)
  }
  ...
  sendVideoMessage(archiveId: string, videoUrl: string) {
    ...
    if (!conversationId && !submissionId) {
      throw new Error('Either a conversationID or a submissionId is required to send a video message')
    }

    return conversationStore.sendVideoMessage(conversationId, submissionId, archiveId, videoUrl).then(() => {
      ...
      .catch(error => {
        ...
        showYesNoAlert({
          ...
          yesTitle: 'Try Again',
          onYes: this.sendVideoMessage.bind(this, conversationId, submissionId, archiveId, videoUrl),
          onNo: () => {
            this.videoMessageSendComplete = true
            this.onFinished(archiveId, videoUrl)
          }
  ...
  render() {
    ...
    return (
      <ConexusLightbox closeable={closeable} hideHeader={true} verticalPercent={1} horizontalPercent={1} style={styles.lightbox}>
        {showRecorder && <ConexusVideoRecorder
          style={{ flex: 1, width: AppSizes.screen.width, height: AppSizes.screen.height }}
        ...
        {showPlayer && !!videoUrl && <ConexusVideoPlayer
          mediaUrl={videoUrl}
          ...
          actionButton={{ title: this.finishedButtonTitle, onPress: this.onFinished.bind(this, this.state.archiveId, this.state.videoUrl) }}
          onLoad={() => log.info('Video Loaded')}
          onError={this.onPlayError.bind(this)}
          style={{ flex: 1 }}
          overlayFooterStyle={styles.overlayFooter} />
        }
        {loading && <ActivityIndicator color={AppColors.blue} style={{ flex: 1 }} />}
      </ConexusLightbox>
    )
  }
}

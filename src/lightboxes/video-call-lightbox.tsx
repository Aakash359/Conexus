...
import { StyleSheet, View, Animated, TouchableOpacity, Easing, InteractionManager, LayoutAnimation } from 'react-native'
import { H3, H1, Header, Icon } from 'native-base'
import { ActionButton, Avatar, KeepAwake, Circle, ScreenFooterButton } from '../components'
...
@inject('videoStore', 'userStore', 'deviceStore')
@observer
export class VideoCallLightbox extends React.Component<VideoCallLightboxProps, VideoCallLightboxState> {
    ...
    constructor(props: VideoCallLightboxProps, state: VideoCallLightboxState) {
        super(props, state)
        this.state = { answer: props.videoStore.isAutoAnswer || props.autoAnswer || props.isOutgoing || false, 
    ...
    componentWillMount() {
        ...
        this._disposePatchListener = onPatch(this.props.videoStore, (patch, old) => {
            this.initCall()
        })
    ...
    _disconnect() {
        ...
        this.props.videoStore.endCall(this.props.userStore.user.userId).then((results) => {
            log.info(results)
    ...
    initCall() {
        ...
        let { isPublishing, isSubscribed, inCall, isAutoAnswer } = this.props.videoStore
        let { isIncoming, answer, status } = this.state

        log.info('VideoCallLightbox:initCall', !this.state.didBothPartiesConnect, isAutoAnswer, inCall, isPublishing, isSubscribed)
        ...
        if (!this.state.didBothPartiesConnect && isPublishing && isSubscribed) {
            ...
            log.info('VideoCallLightbox:startAnimating', !this.state.didBothPartiesConnect, inCall, isPublishing, isSubscribed)
            const cWidth = Math.floor(AppSizes.screen.width * .25)
    ...
    renderCallWaiting() {
        ...
        let { isIncoming, answer, status } = this.state
        return (
            <Animated.View style={StyleSheet.flatten([styles.callWaitingScreen, !answer && { backgroundColor: 'black' }])}>
    ...
    renderPublisher() {
        ...
        return (
            <Animated.View style={[styles.publisherContainter, { height: h, width: w, marginTop: mt, marginLeft: ml, zIndex: (didBothPartiesConnect) ? 99 : 10 }]}>
    ...
    render() {
        let { sessionId, isConnected, setIsSubscribed, setIsPublishing, inCall, isPublishing, isSubscribed, caller } = this.props.videoStore
        const { answer, isIncoming, didBothPartiesConnect } = this.state
        log.info('render', inCall, isPublishing, isSubscribed)
        return ((!!caller) ?
            <ConexusLightbox style={styles.lightbox} verticalPercent={1} horizontalPercent={1}>
                {(isConnected) &&
                    <View style={{ flex: 1, ...StyleSheet.absoluteFillObject, backgroundColor: 'black' }}>
                        ...
                        /> */}
                        {(didBothPartiesConnect) && <View style={styles.inCallButtons}>
                            <ActionButton primary style={styles.endCallButton} title="End Call" onPress={this._disconnect.bind(this)}></ActionButton>
                        </View> }
                    </View>}
                {(isConnected) && this.renderPublisher()}
                {(!didBothPartiesConnect) && this.renderCallWaiting()}
                {(answer && !didBothPartiesConnect) && 
                    <ScreenFooterButton title="Cancel" onPress={this._disconnect.bind(this)} danger hideGradient={true} />
                }
            </ConexusLightbox> : null
        )
    }
}
